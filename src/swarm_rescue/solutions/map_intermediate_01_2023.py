...
from .walls_intermediate_map_1_2023 import add_walls, add_boxes


class MyMapIntermediate01_2023(MapAbstract):

    def __init__(self, zones_config: ZonesConfig = ()):
        super().__init__(zones_config)
        self._time_step_limit = 2700
        self._real_time_limit = 270

        # PARAMETERS MAP
        self._size_area = (800, 500)

        self._rescue_center = RescueCenter(size=(87, 167))
        self._rescue_center_pos = ((346, 156), 0)

        self._no_gps_zone = NoGpsZone(size=(792, 164))
        self._no_gps_zone_pos = ((3, -167), 0)

        self._wounded_persons_pos = [(-284, 188)]
        self._number_wounded_persons = len(self._wounded_persons_pos)
        self._wounded_persons: List[WoundedPerson] = []

        orient = random.uniform(-math.pi, math.pi)
        self._drones_pos = [((245, 160), orient)]
