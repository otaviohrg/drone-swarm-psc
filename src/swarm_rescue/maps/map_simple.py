""" 
    Simple map: Just the rescue center, one wounded person and one drone.
"""
import math
import random
...
class MyMapSimple(MapAbstract):
    def __init__(self, zones_config: ZonesConfig = ()):
        ...
        # Drone
        orient = random.uniform(-math.pi, math.pi) # Random orientation
        self._drones_pos = [((295, 118), orient)] 
        self._number_drones = len(self._drones_pos)
        self._drones: List[DroneAbstract] = []

    def construct_playground(self, drone_type: DroneAbstract) -> Playground:
        ...
        playground.add(self._rescue_center, self._rescue_center_pos) # Add the rescue center to the playground


        self._explored_map.initialize_walls(playground)

        # POSITION OF THE WOUNDED PERSON
        for i in range(self._number_wounded_persons):
            wounded_person = WoundedPerson(rescue_center=self._rescue_center)
            self._wounded_persons.append(wounded_person)
            pos = (self._wounded_persons_pos[i], 0)
            playground.add(wounded_person, pos) # Add the wounded person to the playground
        ...
        for i in range(self._number_drones):
            ...
            playground.add(drone, self._drones_pos[i]) # Add the drone to the playground
